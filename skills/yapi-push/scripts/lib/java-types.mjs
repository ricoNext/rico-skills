...
const CONTAINER_TYPES = new Set(['List', 'Set', 'Collection', 'Iterable', 'ResponseEntity', 'Optional', 'Map', 'Page', 'IPage']);
...
function annotationStringArg(chunk, name, keys = ['value', 'description']) {
  const matches = [...chunk.matchAll(new RegExp(`@${name}\\b(?:\\s*\\(([^)]*)\\))?`, 'g'))];
  const args = matches.at(-1)?.[1];
  if (!args) {
    return '';
  }

  for (const key of keys) {
    const named = args.match(new RegExp(`\\b${key}\\s*=\\s*"([^"]*)"`));
    if (named?.[1]) {
      return named[1].trim();
    }
  }

  return args.match(/^\s*"([^"]*)"/)?.[1]?.trim() || '';
}

function fieldPreamble(source, fieldIndex) {
  const before = source.slice(Math.max(0, fieldIndex - 1500), fieldIndex);
...
export function extractFields(source) {
  const fields = [];
  const pattern = /\b(private|protected|public)\s+(?:static\s+)?(?:final\s+)?([\w.$]+(?:\s*<[^;=(){}]+>)?(?:\[\])?)\s+(\w+)\s*;/g;
  let match = pattern.exec(source);
  while (match) {
    const isStatic = /\bstatic\s+/.test(match[0]);
    const name = match[3];
    if (!isStatic && name !== 'serialVersionUID') {
      const preamble = fieldPreamble(source, match.index);
      const javadoc = [...preamble.matchAll(/\/\*\*([\s\S]*?)\*\//g)].at(-1)?.[1];
      const lineComment = [...preamble.matchAll(/\/\/\s*(.+)$/gm)].at(-1)?.[1];
      const description =
        annotationStringArg(preamble, 'ApiModelProperty') ||
        annotationStringArg(preamble, 'Schema', ['description', 'title']) ||
        (javadoc ? cleanJavadoc(javadoc) : '') ||
        annotationStringArg(preamble, 'NotBlank', ['message']) ||
        annotationStringArg(preamble, 'NotNull', ['message']) ||
        annotationStringArg(preamble, 'NotEmpty', ['message']) ||
        (lineComment ? lineComment.trim() : '');
      const required =
        /@(NotNull|NotBlank|NotEmpty)\b/.test(preamble) ||
        /required\s*=\s*true/.test(preamble) ||
        /requiredMode\s*=\s*[\w.]*REQUIRED/.test(preamble);
      fields.push({
        description,
        name,
        required,
